...
?>
    ...
    <div class="course_title">
        <h4><b><?php  _e('Đề cương khóa học','vibe'); ?></b></h4>
    </div>
            ...
            foreach($course_curriculum as $lesson){
                if(is_numeric($lesson)){
                    ...
                    if(vibe_validate($free) || ($post->post_author == get_current_user_id()) || current_user_can('manage_options') || $curriculum_course_link){
                        $href=apply_filters('wplms_course_curriculum_free_access','<a href="'.get_permalink($lesson).'?id='.get_the_ID().'">'.get_the_title($lesson).(vibe_validate($free)?'<span class="khoahocxemtruoc">'.__('Xem trước','vibe').'</span>':'').'</a>',$lesson,$free);
                    }
...
//					echo apply_filters('wplms_curriculum_time_filter','<span><i class="icon-clock"></i> '.(isset($hours)?$hours.__(' Hours','vibe'):'').' '.$minutes.' '.__('minutes','vibe').'</span><b>'.((isset($hours) && $hours)?$hours:"00").':'.$minutes.'</b>',$min);
                        ...
                        if($hours>0 && $minutes>0){
                            echo '<span><i class="icon-clock"></i>'.$hours.' giờ '.$minutes.' phút</span>';
                        }else{
                            if($hours>0 && $minutes==0){
                                echo '<span><i class="icon-clock"></i>'.$hours.' giờ</span>';
                            }else{
                                echo '<span><i class="icon-clock"></i>'.$minutes.' phút</span>';
                            }
                        }
                    }
