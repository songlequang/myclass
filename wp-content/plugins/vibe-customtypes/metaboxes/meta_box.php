					...
					switch( $type ) {
						...
						case 'selectcpt':
                            ...
                            if($cache_duration){
	                            ...
	                            if(isset($linkage) && $linkage){
	                            	$linkage_terms = get_the_terms( $post->ID, 'linkage');
	                            ...
	                            $kposts = get_transient($key);
			                    if(!isset($kposts) || !is_array($kposts)){     
								   	$args= array(
	                            	'post_type'=>$post_type,
	                            	'numberposts'=> -1
	                            	);
		                            ...
		                            set_transient($key,$kposts,$cache_duration);
	                        	}	
                        	}else{
						...
						case 'post_list':
							echo '<select name="' . $id . '" id="' . $id . '">
									<option value="">'.__('Select One','vibe-customtypes').'</option>'; // Select One
							if($cache_duration){		
								$key='kposts_'.$post_type;
	                            $instructor_content_privacy = vibe_get_option('instructor_content_privacy');
	                            if($instructor_content_privacy){
						...
						case 'curriculum':
							...
							if($cache_duration){												
	                            ...
	                            if(!isset($posts) || !is_array($posts)){
									...
									$posts = get_posts($args);
		                            set_transient($key,$posts,$cache_duration);
	                        	}
	                        }else{
	                        	$args = array( 'post_type' => $post_type1, 'posts_per_page' => -1 );
								$args = apply_filters('wplms_backend_cpt_query',$args);
