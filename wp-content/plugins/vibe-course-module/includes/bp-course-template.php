        ...
        if(isset($r['filters'])){
            ...
            switch($filter){
                ...
                case 'alphabetical':
                    ...
                    break;
                default:
                    $r['orderby'] = array('menu_order'=>'DESC');
...
function bp_course_get_avatar( $args = array() ) {
    ...
    if(has_post_thumbnail($id)){
        ...
        if(isset($default_course_avatar) && $default_course_avatar){
            $thumb='<a href="'.get_permalink($id).'" title="'.the_title_attribute('echo=0').'" itemprop="image"><img src="'.$default_course_avatar.'" itemprop="image" /></a>';
        }
    }
...
//		$title .= '</a>';
    ...
    // code mới
    $title = '<span class="linkkhoahoc" data-href="'. get_permalink($id) .'">';
    $title .= get_the_title($id);
    $title .= '</span>';
    return $title;
}
...
// tao funtion moi de hien thi danh gia ngoai trang khoa hoc
function bp_course_meta_khoa_hoc() {
    echo bp_course_get_course_meta_khoa_hoc();
}
/*
 *
 * We'll assemble the title out of the available information. This way, we can insert
...
function bp_course_get_course_meta() {
    ...
    //    $meta .= '( <strong itemprop="reviewCount">'.(($count)?$count:'0').'</strong> '.__('Đánh giá','vibe').' )</div>';
    $meta .= '( <strong itemprop="reviewCount">'.(($count)?$count:'0').'</strong> '.' )</div>';
...
//    $meta .='<div class="students"><i class="icon-users"></i> '.$students.' '.__('STUDENTS','vibe').'</div>';
    $meta .='<div class="students"><i class="icon-users"></i> '.$students.' '.'</div>';

    return apply_filters('wplms_course_meta',$meta);
}

// thêm function de hien thi danh gia trang khoa hoc
...
if(!function_exists('bp_course_display_rating')){
    function bp_course_display_rating($reviews){
        $meta = '<div class="star-rating" itemprop="aggregateRating" itemscope itemtype="http://schema.org/AggregateRating">
			<i class="hide" itemprop="ratingValue">'.$reviews.'</i>';
        for($i=1;$i<=5;$i++){
            if($reviews >= 1){
...
if(!function_exists('bp_course_get_course_credits')){
    function bp_course_get_course_credits($args=NULL) {
        ...
        if(vibe_validate($free_course)){
            $credits[]= '<strong itemprop="price">'.apply_filters('wplms_free_course_price',__('FREE','vibe')).'</strong>';
        }else{
        ...
        if(count($credits) > 1 ){
            ...
            /*$credits_html .='<select class="pricing_course">';
            $first = 1;
            foreach($credits as $key=>$credit){
                $credits_html .='<option value="'.$key.'" '.(($first)?'selected="selected"':'').'>&nbsp;'.$credit.'</option>';
                $first=0;
            }
            $credits_html .='</select>';*/
...
if(!function_exists('bp_course_get_user_course_status')){
    function bp_course_get_user_course_status($user_id,$course_id){
        ...
        if(!isset($course_status) || !is_numeric($course_status)){
            ...
            }/*else{ //Force Correct the Course Enabled issue.
					
					$expiry = get_user_meta($user_id,$course_id,true);
					if(isset($expiry) && is_numeric($expiry) && $expiry > time()){
