...
function the_course_button($id=NULL){
    ...
    if(isset($coursetaken) && $coursetaken && is_user_logged_in()){   // COURSE IS TAKEN & USER IS LOGGED IN
        ...
        $extra ='';
        if(isset($pid) && $pid){
            echo '<a href="'.$pid.'" class="'.((isset($id) && $id )?'':'course_button full ').'button">'.apply_filters('wplms_take_this_course_button_label',__('TAKE THIS COURSE','vibe'),$course_id).apply_filters('wplms_course_button_extra',$extra,$course_id).'</a>';
        }else{
...
function get_the_course_details($args=NULL){
    ...
    $course_details = array(
        ...
        'time' => '<li><i class="icon-clock"></i><span class="tongthoigiankhoahoc">'.tongthoigianvideokhoahoc(get_the_ID()).'</span></li>',
        ...
        if(isset($course_curriculum) && is_array($course_curriculum)){
            foreach($course_curriculum as $unit_id){
                if(is_numeric($unit_id)){
                    ...
                    $icon = get_post_meta($unit_id,'vibe_type',true);
                    $class='';$flag=0;
                    if($uid == $unit_id){
                        $class .=' active';
                        $flag = 1;
                    ...
                    if(isset($nextunit_access) && $nextunit_access){
                        ...
                        if($flag)
                            $return .= '<li id="unit'.$unit_id.'" class="unit_line '.$class.' unit'.$unit_id.'"><span></span> <a class="unit" data-unit="'.$unit_id.'">'.get_the_title($unit_id).'</a><i class="course_icon icon-'.$icon.'"></i></li>';
                        else
                            $return .= '<li id="unit'.$unit_id.'" class="unit_line '.$class.' unit'.$unit_id.'"><span></span> <a>'.get_the_title($unit_id).'</a><i class="course_icon icon-'.$icon.'"></i></li>';
...
function bp_course_quiz_auto_submit($quiz_id,$user_id){
    ...
    }else{  // End Auto evaluate and Send notification to instructor
