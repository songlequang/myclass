...
function course_stats_user(){
	...
	if(isset($being) && $being !=''){
      ...
      $complete=$total=count($course_curriculum);
			...
			echo '<span>'.human_time_diff($start,time()).'</span></h6>';
            ...
            if(!isset($pre_unit_time) || $pre_unit_time ==''){
             ...
             if(isset($pre_unit_time) && $pre_unit_time){
               ...
               if($value > current_time('timestamp')){
                      echo '<div class="message"><p>'.__('Unit will be available in ','vibe').tofriendlytime($value-current_time('timestamp')).'</p></div>';
                      die();
                  }else{
                      $pre_unit_time=get_post_meta($units[$unitkey],$user_id,true);
                      if(!isset($pre_unit_time) || $pre_unit_time ==''){
