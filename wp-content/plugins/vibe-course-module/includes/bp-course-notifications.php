...
class bp_course_notifications{
		...
		if(isset($lms_settings) && isset($lms_settings['touch'])){
			$settings=array(
				...
				'course_subscribed'=>array(
									'label' => __('Course Subscribed','vibe'),
				...
				'course_added'=>array(
									'label' => __('User added to Course','vibe'),
									'name' =>'course_added',
									'type' => 'touchpoint',
				...
				'course_start'=>array(
									'label' => __('User Starts a Course ','vibe'),
									'name' =>'course_start',
									'type' => 'touchpoint',
				...
				'course_certificate'=>array(
									'label' => __('Course Certificate','vibe'),
				...
				'course_reset'=>array(
									'label' => __('Course Reset by Instructor','vibe'),
									'name' =>'course_reset',
									'type' => 'touchpoint',
				...
				'course_retake'=>array(
									'label' => __('Course Retake by User','vibe'),
									'name' =>'course_retake',
									'type' => 'touchpoint',
				...
				'course_evaluation'=>array(
									'label' => __('Course Evaluation','vibe'),
				...
				'course_unsubscribe'=>array(
									'label' => __('Unsubscribe Course','vibe'),
				...
				'unit_complete'=>array(
									'label' => __('Unit marked complete by User','vibe'),
									'name' =>'unit_complete',
									'type' => 'touchpoint',
				...
				'unit_comment'=>array(
									'label' => __('Unit comment added by User','vibe'),
									'name' =>'course_review',
									'type' => 'touchpoint',
				...
				'quiz_start'=>array(
									'label' => __('Quiz Start by user','vibe'),
									'name' =>'quiz_start',
									'type' => 'touchpoint',
				...
				'quiz_submit'=>array(
									'label' => __('Quiz Submitted by user','vibe'),
									'name' =>'quiz_submit',
									'type' => 'touchpoint',
				...
				'quiz_reset'=>array(
									'label' => __('Quiz Reset by Instructor','vibe'),
									'name' =>'quiz_reset',
									'type' => 'touchpoint',
				...
				'quiz_retake'=>array(
									'label' => __('Quiz Retake by User','vibe'),
									'name' =>'course_retake',
									'type' => 'touchpoint',
				...
				'assignment_start'=>array(
									'label' => __('Assignment Start by user','vibe'),
				...
				'assignment_submit'=>array(
									'label' => __('Assignment Submitted by user','vibe'),
				...
				'assignment_evaluation'=>array(
									'label' => __('Assignment Evaluation','vibe'),
				...
				'assignment_reset'=>array(
									'label' => __('Assignment Reset by Instructor','vibe'),
	...
	function run(){
		$lms_settings = get_option('lms_settings');
		if(isset($lms_settings) && isset($lms_settings['touch']) && is_array($lms_settings['touch'])){
	...
	function student_email_course_announcement($course_id,$student_type,$email,$announcement){
		...
		$subject = sprintf(__('Announcement for Course %s','vibe'),get_the_title($course_id));
		bp_course_wp_mail($to,$subject,$announcement,array('student_course_announcements','item_id'=>$course_id,'user_ids'=>$users));
	...
	function instructor_email_course_announcement($course_id,$student_type,$email,$announcement){
		...
		$subject = sprintf(__('Announcement for Course %s','vibe'),get_the_title($course_id));
		bp_course_wp_mail($to,$subject,$announcement,array('student_course_announcement','item_id'=>$course_id,'user_ids'=>$users));
	}
	...
	function student_email_course_news($news_id,$post){
		...
		bp_course_wp_mail($to,$subject,$message,array('student_course_news','item_id'=>$course_id,'user_ids'=>$users));
	}
	...
	function instructor_email_course_news($news_id,$post){
		...
		$subject = sprintf(__('News for Course %s','vibe'),get_the_title($course_id));
		$message = $post->post_title.' <hr> '.$post->post_content.' <a href="'.get_permalink($news_id).'" class="link">'.__('Read more','vibe').'</a>';
		bp_course_wp_mail($to,$subject,$message,array('student_course_announcement','item_id'=>$course_id,'user_ids'=>$users));
	}

	/* ====== WHEN STUDENT SUBSCRIBES A COURSE ====== */
	//student functions
	function student_message_course_subscribed($course_id,$user_id,$group_id){
	...
	function student_notification_course_subscribed($course_id,$user_id,$group_id){
	...
	function student_email_course_subscribed($course_id,$user_id,$group_id){
		...
		if($enable !== 'no'){
			...
			$message = sprintf(__('You\'re subscribed for course : %s','vibe'),'<a href="'.get_permalink($course_id).'">'.get_the_title($course_id).'</a>');
			bp_course_wp_mail($to,$subject,$message,array('student_email_course_subscribed','item_id'=>$course_id,'secondary_item_id'=>$group_id,'user_id'=>$members));
		}

	}

	//Instructor functions
	function instructor_message_course_subscribed($course_id,$user_id,$group_id){
	...
	function instructor_notification_course_subscribed($course_id,$user_id,$group_id){
		$instructor_ids=apply_filters('wplms_course_instructors',get_post_field('post_author', $course_id),$course_id);
		if(!is_array($instructor_ids))
			$instructor_ids = array($instructor_ids);
	...
	function instructor_email_course_subscribed($course_id,$user_id,$group_id){
		...
		$message = sprintf(__('Student %s subscribed for course : %s','vibe'),bp_core_get_userlink($user_id),'<a href="'.get_permalink($course_id).'">'.get_the_title($course_id).'</a>');
		if(count($to))
			bp_course_wp_mail($to,$subject,$message,array('instructor_email_course_subscribed','item_id'=>$course_id,'secondary_item_id'=>$group_id,'user_id'=>$members));
	}
	...
	function student_email_course_added($check_action,$course_id,$members){ 
		...
		if(count($to))
			bp_course_wp_mail($to,$subject,$message,array('student_email_course_added','item_id'=>$course_id,'user_id'=>$members));
	}
	...
	function instructor_email_course_added($check_action,$course_id,$members){ 
		...
		if(count($to))
			bp_course_wp_mail($to,$subject,$message,array('instructor_email_course_added','item_id'=>$course_id,'user_id'=>$members));
			
	}
	/* ====== WHEN STUDENT STARTS A COURSE ====== */

	function student_message_course_start($course_id,$user_id){
		if(did_action('wplms_start_course'))
			return;
		$sender_id=get_post_field('post_author', $course_id);
	...
	function student_email_course_start($course_id,$user_id){
		...
		if($enable !== 'no'){
			...
			bp_course_wp_mail($to,$subject,$message,array('student_email_course_start','item_id'=>$course_id,'user_id'=>$user_id));
		}
	...
	function instructor_email_course_start($course_id,$user_id){
		...
		$message = sprintf(__('Student %s started the course : %s','vibe'),bp_core_get_userlink($user_id),'<a href="'.get_permalink($course_id).'">'.get_the_title($course_id).'</a>');
		if(count($to))			
			bp_course_wp_mail($to,$subject,$message,array('instructor_email_course_start','item_id'=>$course_id,'user_id'=>$user_id));
	}
	...
	function student_email_course_submit($course_id,$user_id){
		...
		if($enable !== 'no'){
			...
			$message = sprintf(__('You\'ve submitted the course : %s','vibe'),'<a href="'.get_permalink($course_id).'">'.get_the_title($course_id).'</a>');
			bp_course_wp_mail($to,$subject,$message,array('student_email_course_submit','item_id'=>$course_id,'user_id'=>$user_id));
		}
	...
	function instructor_email_course_submit($course_id,$user_id){
		...
		$message = sprintf(__('Student %s submitted the course : %s','vibe'),bp_core_get_userlink($user_id),'<a href="'.get_permalink($course_id).'">'.get_the_title($course_id).'</a>');
		if(count($to))			
			bp_course_wp_mail($to,$subject,$message,array('instructor_email_course_submit','item_id'=>$course_id,'user_id'=>$user_id));
	}
	...
	function student_email_course_reset($course_id,$user_id){
		...
		if($enable !== 'no'){
			...
			bp_course_wp_mail($to,$subject,$message,array('student_email_course_reset','item_id'=>$course_id,'user_id'=>$user_id));
		}
	}
	...
	function instructor_email_course_reset($course_id,$user_id){
		...
		if(count($to))			
			bp_course_wp_mail($to,$subject,$message,array('instructor_email_course_reset','item_id'=>$course_id,'user_id'=>$user_id));
	}
	...
	function student_email_course_retake($course_id,$user_id){
		...
		if($enable !== 'no'){
			...
			bp_course_wp_mail($to,$subject,$message,array('student_email_course_retake','item_id'=>$course_id,'user_id'=>$user_id));
		}
	}
	...
	function instructor_email_course_retake($course_id,$user_id){
		...
		if(count($to))			
			bp_course_wp_mail($to,$subject,$message,array('instructor_email_course_retake','item_id'=>$course_id,'user_id'=>$user_id));
	}
	...
	function student_email_course_evaluation($course_id,$marks,$user_id){
		...
		if($enable !== 'no'){
			...
			$message = sprintf(__('You\'ve obtained %s  in Course : %s','vibe'),apply_filters('wplms_course_marks',$marks.'/100',$course_id),' <a href="'.get_permalink($course_id).'">'.get_the_title($course_id).'</a>');
			bp_course_wp_mail($to,$subject,$message,array('student_email_course_evaluation','item_id'=>$course_id,'user_id'=>$user_id,'marks'=>$marks));
		}
	}
	...
	function instructor_email_course_evaluation($course_id,$marks,$user_id){
		...
		$message = sprintf(__('Student %s got %s in Course %s','vibe'),bp_core_get_userlink($user_id),apply_filters('wplms_course_marks',$marks.'/100',$course_id),'<a href="'.get_permalink($course_id).'">'.get_the_title($course_id).'</a>');
		if(count($to))			
			bp_course_wp_mail($to,$subject,$message,array('instructor_email_course_evaluation','item_id'=>$course_id,'user_id'=>$user_id,'marks'=>$marks));
	}
	...
	function student_email_course_badge($course_id,$badges,$user_id,$badge_filter){
		...
		if($enable !== 'no'){
			...
			bp_course_wp_mail($to,$subject,$message,array('student_email_course_badge','item_id'=>$course_id,'user_id'=>$user_id));
		}
	}
	...
	function instructor_email_course_badge($course_id,$badges,$user_id,$badge_filter){
		...
		$message = sprintf(__('Student %s got a Badge in Course %s','vibe'),bp_core_get_userlink($user_id),'<a href="'.get_permalink($course_id).'">'.get_the_title($course_id).'</a>');
		if(count($to))			
			bp_course_wp_mail($to,$subject,$message,array('instructor_email_course_badge','item_id'=>$course_id,'user_id'=>$user_id));
	}
	...
	function student_message_course_certificate($course_id,$pass,$user_id,$passing_filter){
	    ...
	    if(bp_is_active('messages') && function_exists('messages_new_message'))
	      messages_new_message( array('sender_id' => $sender_id, 'subject' => __('You got a new Certifiacte !','vibe'), 'content' => $message,   'recipients' => $user_id ) );
	}
	...
	function student_email_course_certificate($course_id,$pass,$user_id,$passing_filter){
		...
		if($enable !== 'no'){
			...
			bp_course_wp_mail($to,$subject,$message,array('student_email_course_certificate','item_id'=>$course_id,'user_id'=>$user_id));
		}
	}
	...
	function instructor_email_course_certificate($course_id,$pass,$user_id,$passing_filter){
		...
		$message = sprintf(__('Student %s got a Certificate in Course %s','vibe'),bp_core_get_userlink($user_id),'<a href="'.get_permalink($course_id).'">'.get_the_title($course_id).'</a>');
		if(count($to))			
			bp_course_wp_mail($to,$subject,$message,array('instructor_email_course_certificate','item_id'=>$course_id,'user_id'=>$user_id));
	}
	...
	function student_email_course_codes($code,$course_id,$user_id){
		...
		if($enable !== 'no'){
			...
			$subject = sprintf(__('You\'re unsubscribed from course','vibe'),get_the_title($course_id));
			$message = sprintf(__('You\'re unsubscribed from Course %s','vibe'),' <a href="'.get_permalink($course_id).'">'.get_the_title($course_id).'</a>');
			wbp_course_wp_mail($to,$subject,$message,array('student_email_course_codes','item_id'=>$course_id,'user_id'=>$user_id,'code'=>$code));
		}
	}
	...
	function instructor_email_course_codes($code,$course_id,$user_id){
		...
		$subject = sprintf(__('Student applied code forCourse %s','vibe'),get_the_title($course_id));
	...
	function student_message_quiz_evaluation($quiz_id,$marks,$user_id,$max){	
		...
		$message = sprintf(__('You obtained %d out of %s in Quiz : ','vibe'),$marks,$max,'<a href="'.trailingslashit( bp_core_get_user_domain( $user_id )) . BP_COURSE_SLUG. '/course-results/?action='.$quiz_id .'">'.get_the_title($quiz_id).'</a>');
	...
	function student_message_assignment_evaluation($assignment_id,$marks,$user_id,$max){	
		...
		$message = sprintf(__('You obtained %d out of %s in Assignment : %s ','vibe'),$marks,$max,'<a href="'.trailingslashit( bp_core_get_user_domain( $user_id )) . BP_COURSE_SLUG. '/course-results/?action='.$assignment_id .'">'.get_the_title($assignment_id).'</a>');
	    if(bp_is_active('messages'))
	...
	function student_email_assignment_evaluation($assignment_id,$marks,$user_id,$max){	
		...
		if($enable !== 'no'){
			...
			$message = sprintf(__('You obtained %d out of %s in Assignment : ','vibe'),$marks,$max,'<a href="'.trailingslashit( bp_core_get_user_domain( $user_id )) . BP_COURSE_SLUG. '/course-results/?action='.$assignment_id .'">'.get_the_title($assignment_id).'</a>');
	...
	function instructor_message_assignment_submit($assignment_id,$user_id){	
		...
		$course_id = get_post_meta($assignment_id,'vibe_assignment_course',true);
	      ...
	      messages_new_message( array('sender_id' =>  $user_id, 'subject' => sprintf(__('Assignment %s submitted by Student','vibe'),get_the_title($assignment_id)), 'content' => $message,   'recipients' => $instructor_id) );
	}
	...
	function instructor_emails($course_id,$key){
		...
		foreach($instructor_ids as $instructor_id){
			$enable = get_user_meta($user_id,'instructor_'.$key,true);
			if($enable !== 'no'){
				$user = get_user_by( 'id', $user_id);
				$to[] = $user->user_email;
			}
