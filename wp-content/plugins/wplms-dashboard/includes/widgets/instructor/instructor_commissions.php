...
class wplms_instructor_commission_stats extends WP_Widget {
    ...
    function wplms_instructor_commission_stats() {
  ...
  ",$user_id));
    ...
    if(count($instructor_courses)){
      ...
      if(is_array($product_sales) && $i){
        ...
        if(isset($commissions_paid) && is_Array($commissions_paid)){
          foreach($commissions_paid as $commission){
             if(isset($commission->meta_value[$user_id]) && $commission->meta_value[$user_id]['commission']){
                $flag=1;
