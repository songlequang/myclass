...
class wplms_dash_contact_users extends WP_Widget {
    ...
    function wplms_dash_contact_users() {
    ...
    $this->WP_Widget( 'wplms_dash_contact_users', __(' DASHBOARD : Contact Form', 'wplms-dashboard'), $widget_ops, $control_ops );
    ...
    function get_course_students(){
      ...
      $course_students = $wpdb->get_results("
        SELECT rel.meta_key as user_id
          FROM {$wpdb->postmeta} as rel
          WHERE  rel.post_id  IN ($course_ids_string)
    ...
    function dash_contact_message(){
      ...
      if ( !$members || !$subject || !$message){ echo $members.' - '.$subject.' - '.$message;
             _e('Please enter to/subject/message','wplms-dashboard');
             die();
      }
