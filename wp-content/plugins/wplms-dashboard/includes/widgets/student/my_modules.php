...
class wplms_dash_mymodules extends WP_Widget {
    ...
    function wplms_dash_mymodules() {
    $widget_ops = array( 'classname' => 'wplms_dash_mymodules', 'description' => __('My Courses, Units, Quizzes, Assignments widget for Dashboard', 'wplms-dashboard') );
    $control_ops = array( 'width' => 300, 'height' => 350, 'id_base' => 'wplms_dash_mymodules' );
    $this->WP_Widget( 'wplms_dash_mymodules', __(' DASHBOARD : My Modules Widget', 'wplms-dashboard'), $widget_ops, $control_ops );
  }
      ...
      echo '<div id="tab-myquizzes" class="tab-pane">';
        ...
        if(count($quizzes)){
          ...
          foreach($quizzes as $quiz){
              echo '<li><a href="'.get_permalink($quiz->ID).'">'.get_the_title($quiz->post_title).'</a></li>';
          }
          echo '</ul>';  
        }else{
       ...
       echo '<div id="tab-myassignments" class="tab-pane">';
        ...
        if(count($assignments)){
          ...
          foreach($assignments as $assignment){
              echo '<li><a href="'.get_permalink($assignment->ID).'">'.get_the_title($assignment->post_title).'</a></li>';
          }
          echo '</ul>';  
        }else{
      ...
      echo '<div id="tab-myunits" class="tab-pane">';
        ...
        if(count($units)){
          ...
          foreach($units as $unit){
              echo '<li><a href="'.get_permalink($unit->ID).'">'.get_the_title($unit->post_title).'</a></li>';
          }
          echo '</ul>';  
        }else{
      ...
      echo '<div id="tab-mydownloads" class="tab-pane">';
        ...
        if(count($downloads)){
          ...
          foreach($downloads as $download){
              echo '<li><a href="'.wp_get_attachment_url($download->ID).'" target="_blank">'.get_the_title($download->post_title).'</a></li>';
          }
          echo '</ul>';  
        }else{
